...
- 修复位置沿用 [通用选择原则](diagnosis-and-repair.md#选择修复位置)。`cannot find symbol` 可来自源码引用、依赖可见性或生成内容；参数/签名不匹配先核对实际编译使用的类和版本，不把报错文本当成修法。
...
参数示例：若删去显式 slot 后将依赖 `record.slot`，应追溯本次输入构造和值的来源。旧实现的运行时前提可能与新调用直接相关；修复前不能编译并不能证明该前提在修复后成立。修复方向仍由变更目标决定，不能将本例固定为“删除参数”或“扩展接口”的模板。
...
- 上述 slot 示例可令显式值与输入字段/默认值不同，检查实际路由值；重新编译本身不能区分正确传值与丢失参数。适用的行为和兼容性检查由通用验证规则确定。
