...
- 在原始 CI 结论已经固化后才开始的末端修复，只要所需 Provider 能力存在，就必须使用 `remote_verify`；它不能改写原始结论。
...
5. 在隔离工作区中从同一个干净基线创建每个候选。证据可以跨 Attempt 累积，但不能带入失败候选的代码状态。
6. 执行声明过的验证阶梯。远端模式下，只发布已经通过本地守卫的候选，然后将观察到的流水线结果绑定到其准确的候选 SHA 和 fingerprint。
...
- 不要把用户工作区的修改带入候选。工作区脏或候选可能与用户工作重叠时，使用隔离环境。
