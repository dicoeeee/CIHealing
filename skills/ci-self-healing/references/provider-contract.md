...
本文件约束当前调用所需的 Provider 观察、候选发布与验证动作。按实际工具、权限和执行证据核对能力，只声明有证据支持的范围；平台名称或工具存在不证明该平台已获完整支持。
...
按当前缺口和所选模式核对已有能力，不要求每次具备或执行表中全部动作：
...
- 核对宿主已提供凭据的目标仓库、专用候选命名空间及允许动作；权限不足按能力缺口处理，不自行配置凭据或扩大访问范围。
- 将读取、候选发布、流水线触发、合并和部署保持为不同能力。本 Skill 只消费所选模式要求的前三种能力。
- 发布与触发仅在 [远端执行授权](remote-verification.md#授权与能力)满足后进行；能力协商不授予写权限。

## 适配器验收证据

复用宿主提供的既有验收或运行证据时，核对其 Provider、主机、版本、权限范围与所需能力是否适用；当前目标的身份和实际动作结果仍须按本合同核对。仅有日志读取证据时，只能声明该读取范围已验证，不据此声明支持远端发布或完整 `remote_verify`。

没有相关证据时如实记录未验证范围；影响当前必需动作的能力缺口按[能力协商](#能力协商)处理。当前修复只核对和使用所需能力，不承担新增适配器、编写接入文件或执行平台完整验收；也不为补齐验收证据额外发布候选或触发流水线。
