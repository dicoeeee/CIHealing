...
纯 Markdown 接入即可，不要求动态插件、自动注册或配置扫描程序。文件至少覆盖：

| 内容 | 说明 |
| --- | --- |
| 身份与范围 | Provider ID、系统/主机、适用版本或说明来源 |
| 本地证据 | 实际文件布局与 metadata 对应关系，可无固定布局 |
| MCP | 可识别的服务归属、能力和身份参数；未提供则注明 |
| CLI | 真实可执行文件、可用性检查、有依据的只读命令；未提供则注明 |
| 绑定与完整性 | run/Job/重试定位、分页、截断和日志可用条件 |
...
结果复用 `diagnosis.classification`，最终进入现有 `analysis-fix.md` 的“失败与根因”和“剩余问题与交接”，不生成第二份分类报告。例如：

~~~text
使用标准：<实际 ID、版本、入口>
绑定来源：<SKILL.md 固定绑定区，或未固定时的可信调用>
当前失败：<仓库及 run/Job/重试/命令，或本地失败引用>
已支持路径：<有证据支持的阶段 → 大类，可继续到小类>
分类依据：<原始位置、具体判据及满足关系>
候选/共存与缺口：<A 或 B、标准允许 A 和 B、规则冲突，或缺少的证据>
调查入口与修正：<需要的指南；若有变化，说明证据及下游影响>
分类要求完成情况：<已完成，或尚缺的分类交付项>
~~~

分类可以支持父类而保留小类候选，也可以按标准允许多个类别共存；证据不足与真正未匹配分别报告，判据见[判断与多类别关系](references/failure-classification.md#判断与多类别关系)。
