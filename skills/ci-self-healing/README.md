...
这是 Skill 改造前阅读的维护说明，用于了解能力、调用、定制与交付约定。Agent 仅在明确进行本 Skill 改造时读取本文；诊断、分类、修复、验证及报告等日常执行均不读取本文，也不将其作为运行规则来源。正式执行规则以同目录的 [SKILL.md](SKILL.md) 及其按需引用的执行资源为准。
...
企业适配器可以通过经过认证的 CLI、API、MCP Server、webhook 负载或任务运行器事件流实现。维护者为实际需要的接入编写 reference 或确定性辅助工具，不预建空 Provider 文件或通用命令目录。凭据采用最小权限、短时有效策略，限定目标仓库、专用候选命名空间和所需动作；这些建设与配置工作不由运行中的修复任务承担。
...
只有真实执行证据才能支持对应能力已验收；运行时按[Provider 合同](references/provider-contract.md)使用已有能力和核对证据，不自行补做接入建设或完整验收。

### 完整远端适配器验收

维护者在声明适配器具备完整远端验证能力之前，使用获准的验收环境至少测试：
...
文档示例、工具存在和实际验收分别说明；仅验证日志读取时，只声明该读取能力已验证，不声明完整远端支持。验收结果需保留适用系统、版本、权限与证据，供后续调用核对；单次 CI 修复不承担这套完整验收。
...
清理未完成时仍会输出报告或按已有通道请求协作，但不会报告完整修复成功或“可安全提取 Patch”。候选被否决后按介入时内容恢复，不为匹配编译前清单而重新引入被否决修改；具体结束条件见[结束与协作](references/workspace-cleanup.md#结束与协作)。
